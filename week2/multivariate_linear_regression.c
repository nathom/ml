...
main()
{
    ...
    struct grad g = compute_gradient(x_train, y_train, optimal_w, optimal_b);
...
struct grad
compute_gradient(const matrix x_train, const matrix y, const matrix w, const double b)
{
    double dj_db = 0.0;
    ...
    dj_db /= x_train.rows;
    matrix_scalar_multiply_ip(dj_dw, 1.0 / x_train.rows);
